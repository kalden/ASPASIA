 ...
 * 
 * \brief Package containing utilities to process parameter value sets generated by spartan, creating simulation parameter scripts for each parameter value set, or to introduce interventions into an SBML Model
 */
package modelCreation;
...
/**
 * \brief Create simulation parameter files for efast analysis.
 * 
 * Steps in this process: 1. Generate efast samples for a subset of simulation
 * parameters using spartan (creates a file for each parameter) 2. Set the
 * location of the folder containing these CSV files, the names of the
 * parameters of interest, a location of a parameter file at which parameters
 * are at calibrated values, the number of resample curves, and a location for
 * the output files, in the main method below 3. Run this java program. A
 * parameter file will be created for each sample set in the analysis, with
 * parameters of interest at their values set by the spartan sample, and those
 * not of interest set from their calibrated value
 * 
